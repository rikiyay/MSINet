...
def split_ids(ids):
    ...
    test_ids = rand[i*25:i*25+15]
    val_ids = rand[i*25+15:i*25+25]
    train_ids = list(set(rand.tolist())-set(test_ids[i+1])-set(val_ids[i+1]))

    return train_ids, val_ids, test_ids

class MyDataset(torch.utils.data.Dataset):
    def __init__(self, path2hdf5, df_dict, train_ids, val_ids, test_ids, dset_type, transform=None):
        h5 = h5py.File(path2hdf5)
...
def get_datasets_loaders(path2hdf5, df_dict, train_ids, val_ids, test_ids, augment, transform, batchSize):
    datasets = {}
    loaders = {}

    for dset_type in ['train', 'val', 'test']:
        if dset_type == 'train':
            datasets[dset_type] = MyDataset(path2hdf5, df_dict, train_ids, val_ids, test_ids, dset_type='train', transform = augment)
            loaders[dset_type] = torch.utils.data.DataLoader(datasets[dset_type], batch_size=batchSize, shuffle=True)
        elif dset_type == 'val':
            datasets[dset_type] = MyDataset(path2hdf5, df_dict, train_ids, val_ids, test_ids, dset_type='val', transform = transform)
            loaders[dset_type] = torch.utils.data.DataLoader(datasets[dset_type], batch_size=batchSize, shuffle=True)
        elif dset_type == 'test':
            datasets[dset_type] = MyDataset(path2hdf5, df_dict, train_ids, val_ids, test_ids, dset_type='test', transform = transform)
...
def evaluate(models, datasets, loaders, dataset_sizes, dset_type, criterion):
    prob, label, loss, acc = test_model(model, loaders[dset_type], dataset_sizes[dset_type], criterion)   

    df_tile = pd.DataFrame(columns=['id', 'prob', 'label'])
    if dset_type == 'test':
        df_tile.id = datasets['test'].test_df.ids.values.tolist()
    elif dset_type == 'val':
        df_tile.id = datasets['val'].val_df.ids.values.tolist()
    df_tile.prob = prob
    df_tile.label = label
    ...
    for i in range(len(unique)):
        ave_prob=np.mean(df_tile[df_tile.id==unique[i]].prob.values)
        ave_pred=1 if ave_prob>0.50 else 0
        label=df_tile[df_tile.id==unique[i]].label.values.tolist()[0]
...
if __name__ == '__main__':
    ...
    train_ids, val_ids, test_ids = split_ids(ids)

    datasets, loaders = get_datasets_loaders(dev_path, df_dict, train_ids, val_ids, test_ids, augment, transform, batchSize)
    ...
    torch.save(model.state_dict(), '/path/to/save/state_dict/msi_predictor.pth')
    
    pt_prob_test, pt_pred_test, pt_label_test = evaluate(models, datasets, loaders, dataset_sizes, dset_type='test', criterion=criterion) 

    cm = confusion_matrix(pt_label_test, pt_pred_test)
    print(cm)

    acc = (cm[0][0]+cm[1][1])/len(pt_label_test)*100
    print(f'accuracy = {acc}')

    roc_auc, ci = bootstrap_auc(np.array(pt_label_test), np.array(pt_prob_test))

    # compute operating points
    pt_prob_val, pt_pred_val, pt_label_val = evaluate(models, datasets, loaders, dataset_sizes, dset_type='val', criterion=criterion) 
    
    pt_prob = pt_prob_test + pt_prob_val
    pt_label = pt_label_test + pt_label_val

    fpr, tpr, thresholds  = roc_curve(pt_label, pt_prob)
    
    optimal_idx = np.argmax(tpr - fpr)
    optimal_threshold1 = thresholds[optimal_idx]
    print(f'OP1 is {optimal_threshold1}')

    optimal_idx2 = np.argmax(2*tpr - fpr)
    optimal_threshold2 = thresholds[optimal_idx2]
    print(f'OP2 is {optimal_threshold2}')